...
class StartChatAPIView(APIView):
    ...
    def post(self, request):
        ...
        if usr.is_online == True:
            return Response("User is online and available to chat!", status=status.HTTP_200_OK)
        return Response("User is not online and not available to chat!", status=status.HTTP_400_BAD_REQUEST)
