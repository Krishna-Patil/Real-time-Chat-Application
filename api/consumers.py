...
class ChatConsumer(WebsocketConsumer):
    def connect(self):
        ...
        try:
            c = Channel.objects.create(
                user_id=self.scope["user"].id, name=self.channel_name
            )
            c.save()
            ...
            self.send(json.dumps({"error": "Not a valid token!"}))
            self.close()
    ...
    def receive(self, text_data):
        ...
        try:
            ...
            self.send(json.dumps({"error": "'message' and 'to_user' required!"}))
            self.close()

        try:
            user = CustomUser.objects.get(username=to_user)
        except CustomUser.DoesNotExist:
            self.send(json.dumps({"error": "User does not exist!"}))
            self.close()
        try:
            channel_name = Channel.objects.get(user_id=user.id).name
        except Channel.DoesNotExist:
            self.send(json.dumps({"error": "User is not available!"}))
            self.close()
    ...
    def chat_message(self, event):
        self.send(json.dumps({"from": event["from_user"], "message": event["message"]}))
