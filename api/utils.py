...
def suggest_friends(user_id):
    ...
    user_interests = users_datagram.loc[user_id-1].drop(['name', 'age', 'id']).dropna().sort_values(ascending=False)
    ...
    conditions = [f"`{str(col)}` >= {user_interests_dict[str(col)]}" for col in df1.columns]
    ...
    recommended_datagram['sort'] = pd.Categorical(recommended_datagram['id'], categories=recommended, ordered=True)
