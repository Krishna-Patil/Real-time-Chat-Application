...
def chatpage(request, id):
    ...
    conversation_id = "".join(
        sorted((user.email).replace("@", "-") + (request.user.email).replace("@", "-"))
    )
...
def chat_ch(request, conversation_id):
    ...
    format = "%d-%b-%Y %I:%M %p"
    messages = [
        f"{(msg.timestamp).strftime(format)} {msg.text} -- {msg.from_user}"
        for msg in messages
    ]
    context = {"usr": user, "messages": messages, "room_name": conversation_id}
